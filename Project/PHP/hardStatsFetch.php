<?php
        ...
        $stmt = mysqli_prepare($conn,"SELECT * FROM userstatshard WHERE UserID = ? ORDER BY WPM DESC LIMIT 1");
      ...
      $stmt2 = mysqli_prepare($conn, "SELECT users.Username, userstatshard.UserID, MAX(userstatshard.WPM) AS BestWPM
FROM userstatshard
...
ORDER BY BestWPM DESC ");
      ...
      $stmt3 = mysqli_prepare($conn,"SELECT * FROM userstatshard WHERE UserID = ?");
